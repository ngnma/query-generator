...
# ── Mock backend functions (remove when real backend is ready) ───
def generate_sql(question, schema):
    return "SELECT REGION, SUM(SALES) AS total_sales FROM AMAZON_SALES GROUP BY REGION"
...
def run_sql(sql):
    ...
    data = {
        ...
        "CATEGORY": ["Technology", "Furniture", "Office Supplies", "Furniture", "Technology"],
        "SALES":    [1200.00, 450.00, 30.00, 800.00, 600.00],
        "QUANTITY": [3, 2, 10, 1, 4],
        "PROFIT":   [300.00, 90.00, 5.00, 160.00, 120.00],
    }
